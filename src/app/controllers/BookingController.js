...
const autoCancelUnpaidBookings = async () => {
    ...
    try {
        ...
        if (expiredBookings.length > 0) {
            console.log(
                ` Xóa ${expiredBookings.length} booking không cọc sau 12 giờ.`,
            );
...
module.exports = {
    ...
    create: async (req, res) => {
        ...
        try {
            ...
            req.flash(
                'successMessage',
                'Đặt phòng thành công! Vui lòng thanh toán cọc để xác nhận.',
            );
    ...
    getById: async (req, res) => {
        try {
            ...
            if (!booking)
                return res
                    .status(404)
                    .json({ message: 'Không tìm thấy booking' });
            res.json(booking);
        } catch (err) {
    ...
    checkMyBookings: async (req, res) => {
        if (!req.user) {
            return res
                .status(401)
                .json({ message: 'Bạn cần đăng nhập để xem đặt phòng.' });
        }
